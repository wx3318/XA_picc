...
/*    $('#btn-search-tp').on('click', function() {
        // 重置table查询
    	tabletplwl.ajax.reload();
    });*/
    
    // 绑定重置查询事件
    $('#btn-reset-tp').on('click', function() {
        // 重置table条件
       $("#search-form-tp")[0].reset();
        $("#damageCode").attr('damageCode','');  
        // 重置table查询
    	tabletplwl.ajax.reload();
    });
    //导出excel表
    $('#btn-excel-tp').on('click',function(){   	
    	exportDayExceltp();
    });

    
    // 数据加载url
    var url = "/PICCproject/picc/pending/tplwl.ajax";
    
    // table分页，查询初始化
    tabletplwl = $('#tpLiTable').DataTable({
        ajax: {
            url: url,
            type:"post",
    ...
    function exportDayExceltp() {
    	location.href="/PICCproject/picc/pending/pendingexcel.ajax?areaType=3";
    } 
})