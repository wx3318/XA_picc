...
$(function () { 
    ...
    // 绑定查询事件
/*    $('#btn-search-jiaono').on('click', function() {
        // 重置table查询
    	tablejno.ajax.reload();
    });*/
    
    // 绑定重置查询事件
    $('#btn-reset-jiaono').on('click', function() {
        // 重置table条件
       $("#search-form-jiaono")[0].reset();   
        // 重置table查询
    	tablejno.ajax.reload();
    });
    //导出excel表
    $('#btn-excel-jiaono').on('click',function(){   	
    	exportExceljiaono();
    });

    
    // 数据加载url
    var url = "/PICCproject/picc/pending/jxno.ajax";
    
    // table分页，查询初始化
    tablejno = $('#jiaoNoTable').DataTable({
        ajax: {
            url: url,
            type:"post",
    ...
    function exportExceljiaono() {
    	location.href="/PICCproject/picc/pending/pendingexcel.ajax?areaType=7";
    } 
})