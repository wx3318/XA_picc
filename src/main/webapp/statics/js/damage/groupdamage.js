...
//导出
  $('#btn-excel-day').on('click', function() {
	var groupId = $("#FirstCate").val();
    var param = $("#param").val();
	var damageDate = $("#damageDate").val(); 
	if(""!=groupId && ""!=param && ""!=damageDate){
		location.href="/PICCproject/picc/damage/damagegroupexcel.ajax?groupId="+groupId+"&param="+param+"&damageDate="+damageDate;
	}else{
		bkyyAlert("参数不能为空！！");
	}
  ...
  $("#btn-search-day").on("click",function () {
	 ...
	 if(""!=groupId && ""!=param && ""!=damageDate){
		 $.ajax({  
	         type:"post",         //请求方式  
	         url:"/PICCproject/picc/damage/groupdamagelist.ajax",      //连接  
	         data:"groupId="+groupId+"&&param="+param+"&&damageDate="+damageDate,  //数据加载  
	         dataType:"json", //数据类型  
