...
$(function () { 
  ...
  /*  $('#btn-search-tp').on('click', function() {
        // 重置table查询
    	tablecqlwl.ajax.reload();
    });*/
    
    // 绑定重置查询事件
    $('#btn-reset-cq').on('click', function() {
        // 重置table条件
       $("#search-form-cq")[0].reset();  
        // 重置table查询
        tablecqlwl.ajax.reload();
    });
    //导出excel表
    $('#btn-excel-cq').on('click',function(){   	
    	exportExcelcq();
    });    
    // 数据加载url
    var url = "/PICCproject/picc/pending/cqlwl.ajax";
    
    // table分页，查询初始化
    tablecqlwl = $('#chengLiTable').DataTable({
        ajax: {
            url: url,
            type:"post",           
            /*data:function ( d ) {
    ...
    function exportExcelcq() {
    	location.href="/PICCproject/picc/pending/pendingexcel.ajax?areaType=4";
    } 
})