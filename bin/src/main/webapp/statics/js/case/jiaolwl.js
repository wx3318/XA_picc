...
$(function () { 
  ...
  /*  $('#btn-search-pro1').on('click', function() {
        // 重置table查询
    	tablejlwl.ajax.reload();
    });*/
    
    // 绑定重置查询事件
    $('#btn-reset-jiao').on('click', function() {
        // 重置table条件
        $("#search-form-jiao")[0].reset();    
        // 重置table查询
    	tablejlwl.ajax.reload();
    });
    //导出excel表
    $('#btn-excel-jiao').on('click',function(){   	
    	exportExceljiao();
    });

    
    // 数据加载url
    var url = "/PICCproject/picc/pending/jlwl.ajax";
    
    // table分页，查询初始化
    tablejlwl = $('#jiaoLiTable').DataTable({
        ajax: {
            url: url,
            type:"post",
    ...
    function exportExceljiao() {
    	location.href="/PICCproject/picc/pending/pendingexcel.ajax?areaType=2";
    } 
})