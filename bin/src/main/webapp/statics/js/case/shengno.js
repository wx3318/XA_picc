...
$(function () { 
    ...
    // 绑定查询事件
    /*$('#btn-search-tpno').on('click', function() {
        // 重置table查询
    	tabletpno.ajax.reload();
    });*/
    
    // 绑定重置查询事件
    $('#btn-reset-tpno').on('click', function() {
        // 重置table条件
       $("#search-form-tpno")[0].reset();   
        // 重置table查询
    	tabletpno.ajax.reload();
    });
    //导出excel表
    $('#btn-excel-tpno').on('click',function(){   	
    	exportExceltpno();
    });  
    // 数据加载url
    var url = "/PICCproject/picc/pending/tpno.ajax";
    
    // table分页，查询初始化
    tabletpno = $('#tpNoTable').DataTable({
        ajax: {
            url: url,
            type:"post",
    ...
    function exportExceltpno() {
    	location.href="/PICCproject/picc/pending/pendingexcel.ajax?areaType=6";
    } 
})